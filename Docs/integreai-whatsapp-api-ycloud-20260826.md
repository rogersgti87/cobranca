# IntegreAI WhatsApp API — YCloud (WhatsApp Oficial)

- **Provedor:** `ycloud`
- **Base URL:** `https://integreai.com.br/api`
- **Gerado em:** 2026-08-26 15:11
...
3. Informe `provider` = `ycloud` (query ou body, conforme o endpoint).

## Visão geral

API WhatsApp via YCloud (WhatsApp Oficial).

Autenticação: envie o token Bearer gerado no painel em **API → Tokens**.

Todos os endpoints exigem o campo `provider` com valor `ycloud`.
...
- [`POST /whatsapp/messages/template`](#post-whatsapp-messages-template) — Enviar template oficial (YCloud)
...
- [`GET /whatsapp/templates`](#get-whatsapp-templates) — Listar templates locais (YCloud)
- [`POST /whatsapp/templates`](#post-whatsapp-templates) — Criar template oficial (YCloud)
- [`GET /whatsapp/templates/remote`](#get-whatsapp-templates-remote) — Listar templates remotos na YCloud
- [`GET /whatsapp/templates/{templateId}`](#get-whatsapp-templates-templateid) — Detalhar template local
- [`DELETE /whatsapp/templates/{templateId}`](#delete-whatsapp-templates-templateid) — Excluir template (local + YCloud)
- [`POST /whatsapp/templates/{templateId}/sync`](#post-whatsapp-templates-templateid-sync) — Sincronizar status do template na YCloud

### Webhooks

- [`POST /whatsapp/webhook/ycloud`](#post-whatsapp-webhook-ycloud) — Webhook YCloud
...
|------|----|-------------|-----------|
| `provider` | query | não | Filtrar por provedor (`string`: ycloud|evogo) |
...
|-------|------|-------------|-----------|
| `provider` | string | sim | Enum: `ycloud`, `evogo` |
| `instance` | string | sim |  |
...
<a id="post-whatsapp-messages-template"></a>

#### `POST /whatsapp/messages/template`

**Enviar template oficial (YCloud)**

Somente provider=ycloud. Enfileira mensagem type=template na YCloud. O template precisa existir e estar APPROVED na conta YCloud do cliente.

- **URL:** `https://integreai.com.br/api/whatsapp/messages/template`
- **Autenticação:** Bearer token (Sanctum)
...
| Campo | Tipo | Obrigatório | Descrição |
|-------|------|-------------|-----------|
| `provider` | string | sim | Enum: `ycloud` |
| `instance` | string | sim |  |
| `number` | string | sim |  |
| `template` | object | sim |  |

Exemplo (Template YCloud):
...
```bash
curl -X POST 'https://integreai.com.br/api/whatsapp/messages/template' \
  -H 'Authorization: Bearer SEU_TOKEN' \
  -H 'Accept: application/json' \
  -H 'Content-Type: application/json' \
  -d '{"provider":"ycloud","instance":"ycloud-1","number":"5511999999999","template":{"name":"utilidade_aviso","language":{"code":"pt_BR"},"components":[]}}'
```
...
<a id="get-whatsapp-templates"></a>

#### `GET /whatsapp/templates`

**Listar templates locais (YCloud)**

Somente provider=ycloud. Lista templates salvos na IntegreAI do usuário autenticado (inclui status YCloud).

- **URL:** `https://integreai.com.br/api/whatsapp/templates`
- **Autenticação:** Bearer token (Sanctum)
...
| Nome | Em | Obrigatório | Descrição |
|------|----|-------------|-----------|
| `provider` | query | sim | (`string`: ycloud) |
...
```bash
curl -X GET 'https://integreai.com.br/api/whatsapp/templates?provider=ycloud' \
  -H 'Authorization: Bearer SEU_TOKEN' \
  -H 'Accept: application/json'
```

<a id="post-whatsapp-templates"></a>

#### `POST /whatsapp/templates`

**Criar template oficial (YCloud)**

Somente provider=ycloud. Cria o template local e envia para aprovação na YCloud/Meta (`POST /v2/whatsapp/templates`). Requer integração YCloud conectada.

Paridade com o editor `/app/whatsapp-templates`:
- `type`: `Texto` | `Imagem` | `Documento` (corpo/legenda em `text` ou `body`).
- Cabeçalho texto (`header_text`) só com `type=Texto` (máx. 60). Com Imagem/Documento o HEADER é a mídia (`media_url` HTTPS pública).
- Rodapé opcional (`footer_text`, máx. 60).
- Botão URL opcional (`button_enabled` + `button_text` + `button_url`; `static` ou `dynamic` + `button_url_example`).
- Nome Meta: minúsculas, números e underline.

Após criar, acompanhe aprovação com `POST /whatsapp/templates/{id}/sync` ou webhook `whatsapp.template.reviewed`.
...
|-------|------|-------------|-----------|
| `provider` | string | sim | Enum: `ycloud` |
| `name` | string | sim | Nome Meta: [a-z0-9_]+ |
| `category` | string | não | AUTHENTICATION usa estrutura oficial OTP (não texto livre). UTILITY com texto de código costuma ser REJECTED (INCORRECT_CATEGORY). Enum: `UTILITY`, `MARKETING`, `AUTHENTICATION` Default: `UTILITY` |
| `meta_category` | string | não | Enum: `UTILITY`, `MARKETING`, `AUTHENTICATION` |
| `language` | string | não | Default: `pt_BR` |
| `otp_expiration_minutes` | integer | não | Só AUTHENTICATION — minutos de validade do código no FOOTER Default: `10` |
| `type` | string | não | Tipo do template (igual ao painel). `Texto` = HEADER texto opcional; `Imagem`/`Documento` = HEADER mídia + legenda no BODY. Enum: `Texto`, `Imagem`, `Documento` Default: `Texto` |
| `text` | string | não | Corpo (BODY) obrigatório para Texto; legenda para Imagem/Documento. Use {{1}}, {{2}} para variáveis (amostras enviadas automaticamente à YCloud). |
| `body` | string | não | Alias de text |
| `header_text` | string | não | Cabeçalho opcional (HEADER TEXT). Só aplica com type=Texto. Ignorado em Imagem/Documento (HEADER é a mídia). |
| `footer_text` | string | não | Rodapé opcional (FOOTER). Em category=AUTHENTICATION use otp_expiration_minutes (footer interno). |
| `media_url` | string | não | Obrigatório para Imagem/Documento. HTTPS público usado como example.header_url na YCloud (cópia local fica em /storage/whatsapp/…). |
| `header_url` | string | não | Alias de media_url (amostra HEADER DOCUMENT/IMAGE) |
| `button_enabled` | boolean | não | Se true, inclui componente BUTTONS (URL). Não se aplica a AUTHENTICATION (OTP usa botão COPY_CODE oficial). Default: `false` |
| `button_text` | string | não | Rótulo do botão URL (máx. 25). |
| `button_url` | string | não | URL do botão. Em dynamic a IntegreAI envia base + `/{{1}}` para a YCloud. |
| `button_url_type` | string | não | static = URL fixa; dynamic = sufixo variável {{1}} no final da URL. Enum: `static`, `dynamic` |
| `button_url_example` | string | não | Obrigatório se button_url_type=dynamic — amostra do sufixo (ex.: ABC123), sem colar a URL completa. |
| `create_on_ycloud` | boolean | não | Se true, cria também na YCloud/Meta ao salvar Default: `true` |

Exemplo (Texto + cabeçalho + rodapé + botão estático):
...
```json
{
    ...
    "text": "Olá! Seu pedido {{1}} foi atualizado.",
    "footer_text": "IntegreAI",
    "button_enabled": true,
    "button_text": "Abrir site",
    "button_url": "https://exemplo.com",
    "button_url_type": "static",
    "create_on_ycloud": true
}
```

Exemplo (Com botão URL dinâmica):
...
```json
{
    ...
    "category": "UTILITY",
    "type": "Texto",
    "text": "Acompanhe seu pedido pelo link abaixo.",
    "footer_text": "Atendimento",
    "button_enabled": true,
    "button_text": "Ver pedido",
    "button_url": "https://exemplo.com/pedido",
    "button_url_type": "dynamic",
    "button_url_example": "ABC123",
    "create_on_ycloud": true
}
```

Exemplo (Imagem + legenda + rodapé + botão):
...
```json
{
    ...
    "type": "Imagem",
    "language": "pt_BR",
    "text": "Olá {{1}}! Confira a novidade desta semana.",
    "media_url": "https://exemplo.com/storage/amostras/banner.jpg",
    "footer_text": "Responda SAIR para opt-out",
    "button_enabled": true,
    "button_text": "Ver oferta",
    "button_url": "https://exemplo.com/oferta",
    "button_url_type": "static",
    "create_on_ycloud": true
}
```

Exemplo (Autenticação OTP (oficial Meta)):
...
Exemplo (Documento PDF + legenda + rodapé):

```json
{
    "provider": "ycloud",
    "name": "invoice_new_boleto",
    "category": "UTILITY",
    "type": "Documento",
    "language": "pt_BR",
    "text": "Olá {{1}}! Sua fatura #{{2}} vence em {{3}}.",
    "media_url": "https://exemplo.com/storage/boletos/amostra.pdf",
    "footer_text": "Financeiro",
    "create_on_ycloud": true
}
```
...
```bash
curl -X POST 'https://integreai.com.br/api/whatsapp/templates' \
  -H 'Authorization: Bearer SEU_TOKEN' \
  -H 'Accept: application/json' \
  -H 'Content-Type: application/json' \
  -d '{"provider":"ycloud","name":"aviso_utilidade","category":"UTILITY","type":"Texto","language":"pt_BR","header_text":"Atualização do pedido","text":"Olá! Seu pedido {{1}} foi atualizado.","footer_text":"IntegreAI","button_enabled":true,"button_text":"Abrir site","button_url":"https://exemplo.com","button_url_type":"static","create_on_ycloud":true}'
```

<a id="get-whatsapp-templates-remote"></a>

#### `GET /whatsapp/templates/remote`

**Listar templates remotos na YCloud**

Somente provider=ycloud. Lista templates já existentes na conta YCloud (WABA) do cliente.

- **URL:** `https://integreai.com.br/api/whatsapp/templates/remote`
- **Autenticação:** Bearer token (Sanctum)
...
```bash
curl -X GET 'https://integreai.com.br/api/whatsapp/templates/remote?provider=ycloud' \
  -H 'Authorization: Bearer SEU_TOKEN' \
  -H 'Accept: application/json'
```

<a id="get-whatsapp-templates-templateid"></a>

#### `GET /whatsapp/templates/{templateId}`

**Detalhar template local**

Somente provider=ycloud. Retorna um template local pelo ID.

- **URL:** `https://integreai.com.br/api/whatsapp/templates/{templateId}`
- **Autenticação:** Bearer token (Sanctum)
...
```bash
curl -X GET 'https://integreai.com.br/api/whatsapp/templates/{templateId}' \
  -H 'Authorization: Bearer SEU_TOKEN' \
  -H 'Accept: application/json'
```

<a id="delete-whatsapp-templates-templateid"></a>

#### `DELETE /whatsapp/templates/{templateId}`

**Excluir template (local + YCloud)**

Somente provider=ycloud. Remove o template local. Se estiver vinculado à YCloud, também chama `DELETE /v2/whatsapp/templates/{wabaId}/{name}/{language}` (404 remoto é tratado como sucesso). Bloqueia se o template estiver em uso por campanhas. Use `delete_on_ycloud=false` para remover só o registro local.
...
| `templateId` | path | sim | (integer) |
| `provider` | query | sim | (`string`: ycloud) |
| `delete_on_ycloud` | query | não | Se omitido, exclui na YCloud quando o template tiver vínculo (`ycloud_template_name` / `ycloud_status`). false = só local. (boolean) |
...
```bash
curl -X DELETE 'https://integreai.com.br/api/whatsapp/templates/{templateId}' \
  -H 'Authorization: Bearer SEU_TOKEN' \
  -H 'Accept: application/json'
```

<a id="post-whatsapp-templates-templateid-sync"></a>

#### `POST /whatsapp/templates/{templateId}/sync`

**Sincronizar status do template na YCloud**

Somente provider=ycloud. Consulta o status remoto (PENDING/APPROVED/REJECTED) e atualiza o registro local.

- **URL:** `https://integreai.com.br/api/whatsapp/templates/{templateId}/sync`
- **Autenticação:** Bearer token (Sanctum)
...
```bash
curl -X POST 'https://integreai.com.br/api/whatsapp/templates/{templateId}/sync' \
  -H 'Authorization: Bearer SEU_TOKEN' \
  -H 'Accept: application/json' \
  -H 'Content-Type: application/json' \
  -d '{"provider":"ycloud"}'
...
<a id="post-whatsapp-webhook-ycloud"></a>

#### `POST /whatsapp/webhook/ycloud`

**Webhook YCloud**

Endpoint recebido pela YCloud (`whatsapp.message.updated`, `whatsapp.inbound_message.received`). Eventos são normalizados, com idempotência e opt-out quando aplicável.

- **URL:** `https://integreai.com.br/api/whatsapp/webhook/ycloud`
- **Autenticação:** Bearer token (Sanctum)
...
```bash
curl -X POST 'https://integreai.com.br/api/whatsapp/webhook/ycloud' \
  -H 'Authorization: Bearer SEU_TOKEN' \
  -H 'Accept: application/json'
```
...
| Nome | Em | Obrigatório | Descrição |
|------|----|-------------|-----------|
| `provider` | query | sim | (`string`: ycloud|evogo) |
...
Via IntegreAI Conversation Engine — provider da instância (YCloud (WhatsApp Oficial)).
