...
foreach($invoices as $invoice){
    ...
    public function notificar(Request $request)
    {
        ...
        $invoice = Invoice::with(['customerService.customer', 'company', 'user'])->find($request->input('invoice_id'));

        if (!$invoice || !$invoice->customerService || !$invoice->customerService->customer || !$invoice->company) {
        ...
        if($invoice->status == 'Pendente'){
            ...
            if(Carbon::parse($invoice->date_due)->subDays(5)->format('Y-m-d') == Carbon::now()->format('Y-m-d')){
                if($company->send_generate_invoice == 'Não'){
        ...
        $userImage = $company->logo ?? $invoice->user?->image ?? '';
        ...
        $data = [
            ...
            'logo'                      => 'https://cobrancasegura.com.br/'.$userImage,
